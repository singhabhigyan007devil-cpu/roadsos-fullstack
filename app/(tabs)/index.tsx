...
')}`
  ...
  return (
    ...
    if (panel === 'contacts') {
      return (
        <>
          <Text style={{ color: panelTheme.text, fontSize: 24, fontWeight: '900' }}>Emergency Contacts</Text>
          <Text style={{ color: panelTheme.sub, marginTop: 4 }}>Add trusted SOS contacts for emergency messages and calls.</Text>
              ...
              {countryCodes.map((country) => (
                <TouchableOpacity
                  ...
                  style={{
                    ...
                    borderRadius: 14,
                    backgroundColor:
                      selectedCountry.name === country.name ? '#2563EB' : 'transparent',
                  }}
                >
          ...
          <TouchableOpacity onPress={saveEmergencyContact} style={{ backgroundColor: '#059669', padding: 16, borderRadius: 18, marginTop: 14 }}>
            <Text style={{ color: 'white', textAlign: 'center', fontWeight: '900' }}>Add Contact</Text>
          </TouchableOpacity>

          {savedContacts.length === 0 && (
            <View style={{ backgroundColor: 'rgba(255,255,255,0.06)', padding: 16, borderRadius: 18, marginTop: 14 }}>
              <Text style={{ color: '#CBD5E1', textAlign: 'center' }}>No emergency contacts added yet.</Text>
            </View>
          )}
          ...
          {savedContacts.map((contact) => (
            ...
            >
              <Text style={{ color: panelTheme.text, fontWeight: '700', flex: 1 }}>{contact}</Text>

              <View style={{ flexDirection: 'row' }}>
                <TouchableOpacity onPress={() => Linking.openURL(`tel:${contact}`)} style={{ marginRight: 16 }}>
                  <Text style={{ color: '#38BDF8', fontWeight: '900' }}>CALL</Text>
                </TouchableOpacity>
    ...
    if (panel === 'risk') {
      return (
        <>
          ...
          <View style={{ backgroundColor: panelTheme.chip, padding: 16, borderRadius: 20, marginTop: 14 }}>
            ...
            {riskReasons.length === 0 ? (
              ...
              riskReasons.map((reason) => (
                <Text key={reason} style={{ color: panelTheme.sub, marginTop: 8, lineHeight: 22 }}>
                  • {reason}
                </Text>
              ))
            )}
          ...
          <TouchableOpacity
            onPress={handleSOS}
            style={{ backgroundColor: '#DC2626', padding: 16, borderRadius: 18, marginTop: 16 }}
          >
